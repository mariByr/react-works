...
export const carValidator=Joi.object({
    brand:Joi.string() .required() .pattern(new RegExp('^[a-zA-Zа-яА-яёЁіІїЇєЄҐґ]{3,20}$'))
    .messages({'string.pattern.base':'Your brand value did not match pattern'}),

    price:Joi.number() .required() .min(0).max(10000000).messages({'number.min':'min price is 0',
